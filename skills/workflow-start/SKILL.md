...
编排器管理的经验库采用分层设计，解决"量大了检索慢、命中低、互相矛盾"的问题。

### 三层结构

| 层次 | 数量级 | 性质 | 产生方式 | Agent 使用 |
|------|--------|------|---------|-----------|
| 规则层 Rules | 几十~几百条 | 确定性规则，直接影响行为 | 人工确认后从模式层升级 | 实时加载，直接应用 |
| 模式层 Patterns | 几百~几千条 | 统计规律，待确认 | AI 定时清洗日志聚类分析 | 不直接使用 |
| 原始层 Raw | 全量记录 | 事实记录 | Agent 每次交互自动写入 | 不直接使用 |

### 流转规则

```
原始层 ──AI 清洗聚类──→ 模式层 ──人工确认──→ 规则层 ──→ Agent 决策
```

**只有向上流动，不可跳级**。AI 不能从原始日志直接生成规则。
