...
2. 按 PRD v2.0 模板逐章填:
   - 0 文档元信息 → 填版本号、负责人、状态
   - 📋 AI 友好规范 → **直接复制,不改**(R1-R6 硬规则 + 规范 6 职责边界)
   - 1-7 章(执行摘要 → 功能详述) → 基于 Phase 1 的必要条件填
   - 8 章及以后 → 按需填,不涉及的写"不涉及"
3. 每个**功能模块**按 7.N 五子章节展开（触发条件/交互行为/状态定义/异常处理/明确不做）
...
**🚧 Phase 2 门禁**:
- ✅ 各章节全部存在(可以写"待确认",**不能缺章**)
- ✅ 每个功能模块 5 子章节齐全（触发条件/交互行为/状态定义/异常处理/明确不做）
- ✅ R1-R6 全部遵守(逐条自检)
