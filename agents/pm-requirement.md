...
从 prompt 中提取 Router 派单的三要素,逐条核查:

| 要素 | 预期值 | 缺失时动作 |
|---|---|---|
| 角色 | PM | 反问用户确认角色 |
| 环节 | 需求产出 | 若为 review/验收/复盘 → **拒接**,建议主 Claude 改派对应 Agent |
| 项目类型 | 新项目 or 迭代 | 反问用户二选一 |

**三种入口模式**:

1. **Router 派单**(prompt 含三要素)→ 核查通过直接进 Step 2
2. **用户手动派单**(prompt 缺三要素)→ 反问补齐三要素后进 Step 2
3. **主 Claude 直接调用**(无 Router 上下文)→ 视为手动派单,走模式 2

**拒接条件**(命中任一则退出):
- 环节明确不是需求产出(如"帮我 review 这份 PRD")→ 退出并建议改派
- 用户要求跳过所有门禁直接出 PRD → 退出并说明本 Agent 的工作方式
