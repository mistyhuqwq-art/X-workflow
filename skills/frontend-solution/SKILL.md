...
**职责定位**：技术方案由研发自主产出，PRD 只提供业务规则和验收标准作为输入。研发根据 PRD 的 What 自主决定 How——包括接口设计、数据结构、技术选型、架构分层。PRD 中不应有技术指导，如果 PRD 里有技术实现相关内容，以研发自己的判断为准。

**反面案例**:
- ❌ 直接从 PRD 开始写代码,"边写边设计"
- ❌ 技术方案只写"用 React + Zustand",没有页面拆分和数据流
- ❌ 不考虑和后端的契约,编码时才发现 API 要重新谈
- ❌ 照搬 PRD 里的技术描述，不做独立技术判断
...
每个功能模块的技术方案按以下结构产出（对应 PRD 的每个 7.N 模块）：
...
> 这些内容由研发自主产出，PRD 不涉及。研发根据 PRD 的业务规则和验收标准来设计技术方案。
