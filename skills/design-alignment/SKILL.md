...
**🎯 目标**:动手前**先决定载体**(第 0 步),再确认 5 项对齐清单。载体选错 → 5 项白跑。

### Phase 0.0 — 载体选型(最上游决策)

**核心问题**:这个项目**应该不应该把 Figma 当主战场**?

实战经验:**不是所有项目都适合 Figma 优先**。在没有成熟组件库 + 设计规范 + Design Token 的项目里,硬上 Figma = 「用 Photoshop 画 UI」,每个元素的圆角/间距/颜色都要一个一个拍,没有 Token 约束,改一处变一处。

#### 分场景决策表

| 项目状态 | 推荐载体 | 理由 |
|---|---|---|
| **有成熟组件库 + Token + Figma 库**(如大厂内部系统) | Figma 主战场 | Token 约束 + 组件复用,改动可控 |
| **组件库已定但 Figma 没挂组件库** | **HTML 先做,Figma 补结构图** | HTML 能 1:1 还原视觉,Figma 做页面粒度的结构 review |
| **组件库 / 设计规范都没定**(新项目起步) | **HTML 全程,Figma 先别碰** | 上 Figma = 在白纸上凭感觉画,返工成本高 |
| **纯业务 CRUD 后台**(用 antd / Arco 现成库) | HTML 或 Figma 都行,不做高保真 | 组件都是标准的,不需要像素级还原 |

#### 为什么(踩过的坑反推)

1. **Figma 在无设计系统状态下 = 「用 Photoshop 画 UI」** —— 每个元素的圆角/间距/颜色都要一个一个拍,没有 Token 约束,改一处变一处
2. **MCP 服务端字体不一致** —— 截图经常字体 fallback,验收时反复被误导
3. **Figma 节点 clone 二期组件当「原料」治标不治本** —— 原料是私有的、不是组件库,改不动 variant / state
4. **HTML 对 AI 是「所见即所得」** —— 浏览器渲染 = 真实效果,CSS 错了一眼就看出来,不用靠截图猜
5. **HTML 里能做真交互(hover / 复制按钮 / toast)**,Figma 只能贴死图
6. **HTML 是 CSS 和交互行为的真相源**(workflow.md 里已经过了),Figma 是结构和组件语义的真相源 —— 但**没有组件语义的时候**,Figma 的那半边价值不成立

#### 核心规则

> **在项目没有组件库规范 / 设计规范 / 设计 Token 之前,UI 稿优先用 HTML,不上 Figma。**

#### 🚧 Phase 0.0 门禁

- ✅ 载体选型决策有明确答复 + 理由
- ❌ "先 Figma 画着试试" → 拒绝,没有组件库约束的 Figma = 返工天堂
- ❌ "我们有 antd 还去 Figma 画像素还原" → 拒绝,标准组件库项目不需要高保真还原

**选定载体后**,才进入下面的 5 项对齐清单。注意:**载体决策会影响 design-system / design-prototype 是否需要走完整流程** —— 如果选 HTML,则 design-system(Token 三层)可简化,design-prototype(Mockup + 高保真)的"高保真"阶段由 HTML 替代。
